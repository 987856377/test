...
//        inHospitalDictionaryList.java8.collections.stream().filter(province -> "1".equals(province.getType()))
//                .forEach(province -> {
//                    province.setDictionaryList(inHospitalDictionaryList.java8.collections.stream().filter(item -> "2".equals(item.getType()) && province.getKey().equals(item.getSupervising())).collect(Collectors.toList()));
//                    province.getDictionaryList().forEach(city -> {
//                        city.setDictionaryList(inHospitalDictionaryList.java8.collections.stream().filter(item -> city.getKey().equals(item.getSupervising()) && "3".equals(item.getType())).collect(Collectors.toList()));
//                    });
//        });
