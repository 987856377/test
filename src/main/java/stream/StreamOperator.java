...
//        min：返回流中元素最小值
        ...
        boolean allMatch = list4.stream().allMatch(e -> e > 10); //false
        boolean noneMatch = list4.stream().noneMatch(e -> e > 10); //true
        boolean anyMatch = list4.stream().anyMatch(e -> e > 4);  //true

        Integer findFirst = list4.stream().findFirst().get(); //1
        Integer findAny = list4.stream().findAny().get(); //1

        long count = list4.stream().count(); //5
        Integer max = list4.stream().max(Integer::compareTo).get(); //5
        Integer min = list4.stream().min(Integer::compareTo).get(); //1
...
//         经过测试，当元素个数小于24时，并行时线程数等于元素个数，当大于等于24时，并行时线程数为16
        ...
        Integer v = list5.stream().reduce((x1, x2) -> x1 + x2).get();
        System.out.println(v);   // 300

        Integer v1 = list5.stream().reduce(10, (x1, x2) -> x1 + x2);
        System.out.println(v1);  //310

        Integer v2 = list5.stream().reduce(0,
                (x1, x2) -> {
...
/*
        ...
        //聚合操作
        //1.学生总数
        Long count1 = list.stream().collect(Collectors.counting()); // 3
        //2.最大年龄 (最小的minBy同理)
        ...
        Map<Boolean, List<Student>> partMap = list.stream().collect(Collectors.partitioningBy(v6 -> v6.getAge() > 10));
